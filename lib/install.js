...
import { system, tempDir, fs, logger } from 'appium-support';


const log = logger.getLogger('Chromedriver Install');

const CD_VER = process.env.npm_config_chromedriver_version || process.env.CHROMEDRIVER_VERSION || '2.28';
...
async function installForPlatform (version, platform, arch) {
  if (version === 'LATEST') {
    version = JSON.parse(await request.get({uri: CD_CDN + '/LATEST_RELEASE'}));
  }
