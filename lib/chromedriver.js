...
class Chromedriver extends events.EventEmitter {
  ...
  async getChromedrivers (mapping) {
    ...
    const cds = (await asyncmap(executables, async function (executable) {
      let stdout;
      let errMsg = `Cannot retrieve version string from '${path.basename(executable)}' Chromedriver binary. ` +
        `Make sure it returns a valid version string in response to '--version' command line argument`;
      try {
        ({stdout} = await exec(executable, ['--version']));
      } catch (err) {
        errMsg += `. Original error: ${err.message}`;
        if (err.stderr) {
          errMsg += `. Stderr: ${err.stderr}`;
        }
        log.warn(errMsg);
        return null;
      }
      ...
      if (!match) {
        log.warn(`${errMsg}. Original stdout: ${stdout}`);
        return null;
      }
