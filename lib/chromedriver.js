...
class Chromedriver extends events.EventEmitter {
  ...
  syncProtocol (cdName = null, cdVersion = null) {
    const coercedVersion = semver.coerce(cdVersion);
    if (!coercedVersion || coercedVersion.major < MIN_CD_VERSION_WITH_W3C_SUPPORT) {
      log.debug(`${cdName} v. ${cdVersion} does not fully support ${PROTOCOLS.W3C} protocol. ` +
    ...
    if (chromeOptions.w3c === false) {
      log.info(`${cdName} v. ${cdVersion} supports ${PROTOCOLS.W3C} protocol, ` +
  ...
  syncProtocolIfChromium (out) {
    ...
    // const match = /Starting (ChromeDriver|Microsoft Edge WebDriver) ([.\d]+)/.exec(out);
    const match = /Starting (ChromeDriver|Microsoft Edge WebDriver) ([.\d]+)/.exec(out);
    if (match && match.length === 3) {
      log.debug(`${match[1]} version: '${match[2]}'`);
      this.syncProtocol(match[1], match[2]);
    }
  }
  ...
  async start (caps, emitStartingState = true) {
    ...
    try {
      ...
      this.proc.on('output', (stdout, stderr) => {
        ...
        this.syncProtocolIfChromium(out);
