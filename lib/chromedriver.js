...
function _getChromedriverPathWin()
{
  ...
  var chromedriver = _resolvePath(platform, executable);
  logger.debug("Set chromedriver binary as: " + chromedriver);
  return chromedriver;
}
...
export class Chromedriver extends EventEmitter {
  ...
  constructor (args)  {
    ...
    this.chromedriver = args.executable || getChromedriverPath();
  ...
  static getPath () {
    ...
    assert(ensureExists(path));

    return path;
  };

  async static ensureExists (path) {
