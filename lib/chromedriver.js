...
class Chromedriver extends events.EventEmitter {
  constructor (args = {}) {
    ...
    this.storageClient = isAutodownloadEnabled
      ...
      : null;
    this.capabilities = {};
    this.desiredProtocol = BaseDriver.DRIVER_PROTOCOL.MJSONWP;
  }
  ...
  syncProtocol (cdVersion = null) {
    ...
    if (!coercedVersion || coercedVersion.major < MIN_CD_VERSION_WITH_W3C_SUPPORT) {
      log.debug(`Chromedriver v. ${cdVersion} does not fully support ${BaseDriver.DRIVER_PROTOCOL.W3C} protocol. ` +
        `Defaulting to ${BaseDriver.DRIVER_PROTOCOL.MJSONWP}`);
      return;
    }
    const chromeOptions = getCapValue(this.capabilities, 'chromeOptions', {});
    if (chromeOptions.w3c === false) {
      log.info(`Chromedriver v. ${cdVersion} supports ${BaseDriver.DRIVER_PROTOCOL.W3C} protocol, ` +
        `but ${BaseDriver.DRIVER_PROTOCOL.MJSONWP} one has been explicitly requested`);
      return;
    }
    this.desiredProtocol = BaseDriver.DRIVER_PROTOCOL.W3C;
  ...
  async startSession () {
    const sessionCaps = this.desiredProtocol === BaseDriver.DRIVER_PROTOCOL.W3C
