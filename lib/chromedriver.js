...
var {isLinux, isWindows, isMac, is64bit, is32bit} = require('is-os');

//////////////////////////////////////////////////////

function _resolvePath(platform, executable) {
  // HAH! this won't work, we're not in appium any more Toto.
  return path.resolve(__dirname, "..", "..", "..", "build",
    "chromedriver", platform, executable);
}

function _getChromedriverPathWin() {
  return  _resolvePath('windows', 'chromedriver.exe');
}

function _getChromedriverPathMac() {
  return _resolvePath('mac', 'chromedriver');
}

function _getChromedriverPathLinux() {
  ...
  return _resolvePath('linux', executable);

}


export class Chromedriver extends EventEmitter {
  ...
  constructor (args)  {
    ...
    var chromedriverPath = args.executable || getChromedriverPath();
    this.chromedriver = chromedriverPath;
    logger.debug("Set chromedriver binary as: " + chromedriverPath);
  }

  static async ensureExists (path) {
    logger.debug("Ensuring Chromedriver exists");

    let exists = false;

    try{
    await Q.denodeify(fs.open)(path);
      exists = true;
    } catch (e) {
      logger.debug("Could not find chromedriver.");
    }
  ...
  static getPath () {
    ...
    if (isLinux()) {
      path = _getChromedriverPathLinux();
    } else if (isWindows()) {
      path = _getChromedriverPathWin();
    } else if (isMac()) {
      path = _getChromedriverPathMac();
    }

    assert(Chromedriver.ensureExists(path));
