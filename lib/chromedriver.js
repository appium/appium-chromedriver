...
const CHROMEDRIVER_CHROME_MAPPING = {
  ...
  '2.8': '30.0.1573',
  '2.7': '30.0.1573',
  // still trying to figure out the minimums for 2.6 and 2.5
  '2.6': '',
  '2.5': '',
  '2.4': '29.0.1545',
  '2.3': '28.0.1500',
...
class Chromedriver extends events.EventEmitter {
  ...
  async getCompatibleChromedriver () {
      ...
      try {
        ...
        if (match) {
          ...
          return {
            executable,
            version,
            minCDVersion: CHROMEDRIVER_CHROME_MAPPING[version],
          };
        }
      } catch (ign) {}
    })).filter((cd) => !!cd);
    ...
    for (const cd of cds) {
      log.debug(`    ${cd.executable} (minimum Chrome version '${cd.minCDVersion}')`);
    }

    const workingCds = cds.filter((cd) => semver.gte(chromeVersion, cd.minCDVersion))
