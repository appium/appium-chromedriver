...
class Chromedriver extends events.EventEmitter {
  ...
  syncProtocol(cdVersion) {
    ...
    if (!coercedVersion || coercedVersion.major < MIN_CD_VERSION_WITH_W3C_SUPPORT) {
      this.log.debug(
        `The WebDriver v. ${cdVersion} does not fully support ${PROTOCOLS.W3C} protocol. ` +
    ...
    if (chromeOptions.w3c === false) {
      this.log.info(
        `The WebDriver v. ${cdVersion} supports ${PROTOCOLS.W3C} protocol, ` +
  ...
  detectWebDriverProtocol(out) {
    if (this.driverVersion) {
      // Nothing does if the protocol was already detected
      return;
    }
    ...
    if (match && match.length === 3) {
      ...
      this.syncProtocol(match[2]);
    } else {
      this.log.debug(`No driver version information existed in ${out}`);
    }
  }
