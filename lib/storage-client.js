...
class ChromedriverStorageClient {
  ...
  selectMatchingDrivers (osInfo, opts = {}) {
    ...
    if (!isNaN(minBrowserVersion)) {
      ...
      for (const cdInfo of _.values(this.mapping)) {
        const currentMinBrowserVersion = parseInt(cdInfo.minBrowserVersion, 10);
        if (!isNaN(currentMinBrowserVersion)
          ...
          closestMatchedVersionNumber = minBrowserVersionInt;
        }
      }
      driversToSync = driversToSync.reduce(
        (acc, cdName) => `${this.mapping[cdName].minBrowserVersion}` === `${closestMatchedVersionNumber > 0
          ? closestMatchedVersionNumber
          : minBrowserVersionInt}` ? [...acc, cdName] : acc, []);

      log.debug(`Got ${driversToSync.length} item${driversToSync.length === 1 ? '' : 's'}`);
      if (_.isEmpty(driversToSync)) {
        return [];
      }
      if (closestMatchedVersionNumber > 0 && closestMatchedVersionNumber !== minBrowserVersionInt) {
        log.info(`Cannot find the exact driver match for the major Chrome version ${minBrowserVersionInt}. ` +
          `Will select the most recent driver versioned as '${this.mapping[driversToSync[0]].version}'`);
      }
    }

    if (!_.isEmpty(osInfo)) {
      // Filter out drivera for unsupported system architectures
