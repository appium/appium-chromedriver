...
import {
  ...
  doesCdNameMatchOsNameAndArchitecture,
  GOOGLEAPIS_CDN,
} from './utils';
...
export class ChromedriverStorageClient {
  ...
  async retrieveMapping(shouldParseNotes = true) {
    ...
    this.mapping = await parseGoogleapiStorageXml(xml, shouldParseNotes);
    return this.mapping;
  }
  ...
  selectMatchingDrivers(osInfo, opts = {}) {
    ...
    if (!_.isEmpty(osInfo)) {
      ...
      if (arch === X64 && !driversToSync.some(
        (cdName) => doesCdNameMatchOsNameAndArchitecture(cdName, name, X64)
      )) {
      ...
      driversToSync = driversToSync.filter((cdName) => doesCdNameMatchOsNameAndArchitecture(cdName, name, arch));
      log.debug(`Got ${util.pluralize('item', driversToSync.length, true)}`);
    }
