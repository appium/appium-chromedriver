...
export class ChromedriverStorageClient {
  ...
  async retrieveMapping(shouldParseNotes = true) {
    ...
    ].map(([url, contentType]) => retrieveData(url, {
      'user-agent': USER_AGENT,
      accept: `${contentType}, */*`,
    }, {timeout: this.timeout})));
    this.mapping = await parseGoogleapiStorageXml(xmlStr, shouldParseNotes);
  ...
  selectMatchingDrivers(osInfo, opts = {}) {
    ...
    if (!_.isEmpty(osInfo)) {
      ...
      if (_.isEmpty(result) && arch === ARCH.X64 && (cpu === CPU.INTEL || !cpu)) {
