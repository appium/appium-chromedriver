...
//let proxyTo = require('../common.js').proxyTo
...
export class Chromedriver extends events.EventEmitter {
  ...
  constructor (args)  {
    ...
    let chromedriverPath = args.executable || getPath;
  ...
  start (cb) {
    ...
    let args = ["--url-base=wd/hub", "--port=" + this.proxyPort, "--adb-port=" + ADB.getAdbServerPort()];
  ...
  proxyNewSession (data, cb) {
    ...
    let curRetry = 0;
    let retryInt = 500;
    let doProxy = function (alreadyRestarted) {
  ...
  deleteSession (cb) {
    ...
    let url = '/wd/hub/session/' + this.chromeSessionId;
